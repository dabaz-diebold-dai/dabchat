...
import { ServerWithMembersWithProfiles } from "@dabaz/types";
import { ScrollArea } from "@dabaz/components/ui/scroll-area";
import { UserAvatar } from "@dabaz/components/user-avatar";
...
} from "@dabaz/components/ui/dropdown-menu"
import { MemberRole } from "@prisma/client";

const roleIconMap = {
  "GUEST": null,
  "MODERATOR": <ShieldCheck className="h-4 w-4 ml-2 text-blue-600 dark:text-blue-400" />,
  "ADMIN": <ShieldAlert className="h-4 w-4 ml-2 text-blue-600 dark:text-blue-400" />
}
      ...
      <DialogContent className="bg-neutral-100 dark:bg-neutral-900 text-black dark:text-white overflow-hidden">
        <DialogHeader className="pt-8 px-6">
          ...
          <DialogDescription className="text-center text-neutral-600 dark:text-neutral-400">
            {server?.members?.length} Members are in this server right now.
          </DialogDescription>
        </DialogHeader>
            ...
            <div key={member.id} className="flex items-center gap-x-2 mb-6">
              ...
              {server.profileId !== member.profileId && loadingId !== member.id && (
                <div className="ml-auto">
                  <DropdownMenu>
                    <DropdownMenuTrigger>
                      <MoreVertical className="h-4 w-4 text-neutral-600 dark:text-neutral-400 border-none outline-none" />
                    </DropdownMenuTrigger>
              ...
              {loadingId === member.id && (
                <Loader2
                  className="animate-spin text-neutral-600 dark:text-neutral-400 ml-auto w-4 h-4"
                />
              )}
            </div>
          ))}
